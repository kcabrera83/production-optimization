...
def test_models():
    results = {}
    results['net_profit'] = evaluate_model(os.path.join(MODEL_DIR, 'net_profit_model.pkl'), X, y)
    results['production_efficiency'] = evaluate_model(os.path.join(MODEL_DIR, 'production_efficiency_model.pkl'), X, y)
    return results
